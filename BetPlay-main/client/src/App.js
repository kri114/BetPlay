...
function AdminPanel({ token }) {
  ...
  var [betsTotal, setBetsTotal] = useState(0);
  var [betsPage, setBetsPage] = useState(1);
  var [betStatus, setBetStatus] = useState("");
  var [loading, setLoading]   = useState(false);
  var [error, setError]       = useState(null);
  var [balanceModal, setBalanceModal] = useState(null); // { id, username, balance }
  var [balanceAmt, setBalanceAmt]     = useState("");
  var [balanceReason, setBalanceReason] = useState("");
  var [actionMsg, setActionMsg] = useState(null);

  var adminApi = useCallback(async function(path, opts) {
    opts = opts || {};
    var res = await fetch("/api" + path, Object.assign({}, opts, {
      headers: Object.assign({ "Content-Type": "application/json", "x-admin-secret": "8055" }, { Authorization: "Bearer " + token }, opts.headers || {}),
      body: opts.body ? JSON.stringify(opts.body) : undefined,
    }));
  ...
  useEffect(function() {
    if (tab === "overview") {
      ...
      adminApi("/admin/stats").then(function(d){ setStats(d); setLoading(false); }).catch(function(e){ setError(e.message); setLoading(false); });
    }
    if (tab === "users") {
      setLoading(true); setError(null);
      adminApi("/admin/users").then(function(d){ setUsers(d); setLoading(false); }).catch(function(e){ setError(e.message); setLoading(false); });
    }
    if (tab === "bets") {
      setLoading(true); setError(null);
      adminApi("/admin/bets?page=" + betsPage + "&limit=30" + (betStatus ? "&status=" + betStatus : ""))
        .then(function(d){ setBets(d.bets); setBetsTotal(d.total); setLoading(false); })
        .catch(function(e){ setError(e.message); setLoading(false); });
    }
  }, [tab, betsPage, betStatus]);

  var showMsg = function(msg, color) {
    setActionMsg({ msg, color: color || "#4ade80" });
    setTimeout(function(){ setActionMsg(null); }, 3000);
  };

  var handleBalanceAdjust = async function() {
    var amt = parseFloat(balanceAmt);
    if (isNaN(amt)) return;
    try {
      var d = await adminApi("/admin/users/" + balanceModal.id + "/balance", { method: "POST", body: { amount: amt, reason: balanceReason } });
      showMsg("Balance updated → $" + d.newBalance.toFixed(2) + " for " + d.username);
      setBalanceModal(null); setBalanceAmt(""); setBalanceReason("");
      adminApi("/admin/users").then(function(d){ setUsers(d); });
    } catch(e) { showMsg(e.message, "#f87171"); }
  };

  var handleSettle = async function() {
    try {
      await adminApi("/admin/settle", { method: "POST" });
      showMsg("Settlement triggered successfully");
    } catch(e) { showMsg(e.message, "#f87171"); }
  };

  var fmtAmt = function(n) { return "$" + Number(n||0).toLocaleString(undefined, { minimumFractionDigits:2, maximumFractionDigits:2 }); };
  var fmtDate = function(d) { if (!d) return "—"; return new Date(d).toLocaleDateString("en-GB", { day:"numeric", month:"short", year:"numeric" }); };

  var tabStyle = function(id) {
    return { padding:"7px 14px", borderRadius:8, border:"none", cursor:"pointer", fontWeight:800, fontSize:11, letterSpacing:0.5,
      background: tab===id ? "#e8ff47" : "rgba(255,255,255,0.06)",
      color: tab===id ? "#060d1a" : "rgba(255,255,255,0.5)" };
  };

  var card = function(label, value, color, sub) {
    return (
      <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px", flex:1, minWidth:0 }}>
        <div style={{ fontSize:9, color:"rgba(255,255,255,0.35)", fontWeight:700, letterSpacing:1, marginBottom:6, textTransform:"uppercase" }}>{label}</div>
  ...
  // Mini bar chart
  var BarChart = function({ data, valueKey, labelKey, color }) {
    var max = Math.max(...data.map(function(d){ return d[valueKey] || 0; }), 1);
    return (
      <div style={{ display:"flex", alignItems:"flex-end", gap:3, height:60 }}>
        {data.map(function(d, i) {
          var h = Math.max(3, Math.round((d[valueKey] / max) * 56));
          return (
            <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
              <div title={fmtAmt(d[valueKey])} style={{ width:"100%", height:h, background: color || "#e8ff47", borderRadius:"3px 3px 0 0", opacity: 0.7 + (i/data.length)*0.3 }} />
            </div>
          );
        })}
      </div>
  ...
  var statusColor = function(s) {
    if (s === "won") return "#4ade80";
    if (s === "lost") return "#f87171";
    if (s === "pending") return "#fbbf24";
    if (s === "refunded") return "#60a5fa";
    return "#999";
  };
  ...
  return (
    <div style={{ fontFamily:"inherit" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <div>
          <div style={{ fontSize:18, fontWeight:900, color:"#e8ff47" }}>Admin Panel</div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)", marginTop:1 }}>BetPlay Management Console</div>
        </div>
        <button onClick={handleSettle} style={{ padding:"7px 14px", background:"rgba(96,165,250,0.1)", border:"1px solid rgba(96,165,250,0.3)", color:"#60a5fa", borderRadius:8, fontWeight:800, fontSize:11, cursor:"pointer" }}>
          ↻ Force Settle
        </button>
      </div>
      ...
      <div style={{ display:"flex", gap:5, marginBottom:16 }}>
        {[["overview","Overview"],["users","Users"],["bets","Bets"]].map(function(t){
          return <button key={t[0]} onClick={function(){ setTab(t[0]); }} style={tabStyle(t[0])}>{t[1]}</button>;
        })}
      </div>

      {error && <div style={{ padding:"10px 14px", background:"rgba(248,113,113,0.08)", border:"1px solid rgba(248,113,113,0.25)", borderRadius:10, color:"#f87171", fontSize:12, marginBottom:12 }}>{error}</div>}
      {loading && <div style={{ textAlign:"center", padding:"30px 0", color:"rgba(255,255,255,0.3)", fontSize:13 }}>Loading...</div>}
      ...
      {!loading && tab === "overview" && stats && (
        <div>
          ...
          <div style={{ display:"flex", gap:8, marginBottom:8 }}>
            {card("House Edge", fmtAmt(stats.overview.houseEdge), stats.overview.houseEdge >= 0 ? "#4ade80" : "#f87171")}
            {card("Total Wagered", fmtAmt(stats.overview.totalWagered), "#e8ff47")}
          </div>
          <div style={{ display:"flex", gap:8, marginBottom:8 }}>
            {card("Total Paid Out", fmtAmt(stats.overview.totalPaidOut), "#f87171")}
            {card("Refunded", fmtAmt(stats.overview.totalRefunded), "#60a5fa")}
          </div>
          <div style={{ display:"flex", gap:8, marginBottom:14 }}>
            {card("Users", stats.overview.totalUsers, "#e8ff47", "registered accounts")}
            {card("Total Bets", stats.overview.totalBets, "#fff", stats.overview.pendingCount + " pending")}
          </div>

          {/* Bet status breakdown */}
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:11, fontWeight:800, marginBottom:12, color:"rgba(255,255,255,0.6)", letterSpacing:1 }}>BET STATUS BREAKDOWN</div>
            <div style={{ display:"flex", gap:8 }}>
              {[
                { label:"Won",     count: stats.overview.wonCount,     color:"#4ade80" },
                { label:"Lost",    count: stats.overview.lostCount,    color:"#f87171" },
                { label:"Pending", count: stats.overview.pendingCount, color:"#fbbf24" },
                { label:"Refund",  count: stats.overview.refundedCount,color:"#60a5fa" },
              ].map(function(s){
                var total = stats.overview.totalBets || 1;
                var pct = Math.round((s.count / total) * 100);
                return (
                  <div key={s.label} style={{ flex:1, background:"rgba(255,255,255,0.03)", borderRadius:10, padding:"10px 8px", textAlign:"center" }}>
                    ...
                    <div style={{ fontSize:9, color:s.color, opacity:0.7, marginTop:2 }}>{pct}%</div>
                  </div>
                );
              })}
            </div>
          ...
          {/* Daily revenue chart */}
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:11, fontWeight:800, marginBottom:4, color:"rgba(255,255,255,0.6)", letterSpacing:1 }}>DAILY PROFIT (14 DAYS)</div>
            <div style={{ fontSize:10, color:"rgba(255,255,255,0.25)", marginBottom:10 }}>Wagered minus paid out</div>
            <BarChart data={stats.dailyRevenue} valueKey="profit" color="#e8ff47" />
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
              <div style={{ fontSize:9, color:"rgba(255,255,255,0.2)" }}>{stats.dailyRevenue[0] && stats.dailyRevenue[0].date}</div>
              <div style={{ fontSize:9, color:"rgba(255,255,255,0.2)" }}>{stats.dailyRevenue[stats.dailyRevenue.length-1] && stats.dailyRevenue[stats.dailyRevenue.length-1].date}</div>
            </div>
          </div>

          {/* Daily bets chart */}
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:11, fontWeight:800, marginBottom:4, color:"rgba(255,255,255,0.6)", letterSpacing:1 }}>DAILY BET VOLUME (14 DAYS)</div>
            <BarChart data={stats.dailyRevenue} valueKey="bets" color="#60a5fa" />
          </div>

          {/* Top leagues */}
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:11, fontWeight:800, marginBottom:10, color:"rgba(255,255,255,0.6)", letterSpacing:1 }}>TOP LEAGUES BY VOLUME</div>
            {stats.topLeagues.map(function(l, i){
              var maxW = stats.topLeagues[0] ? stats.topLeagues[0].wagered : 1;
              var pct = Math.max(4, Math.round((l.wagered / maxW) * 100));
              return (
                <div key={l.league} style={{ marginBottom:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:"#fff" }}>{l.league}</div>
                    <div style={{ fontSize:11, color:"#e8ff47", fontWeight:800 }}>{fmtAmt(l.wagered)} <span style={{ color:"rgba(255,255,255,0.3)", fontWeight:400 }}>({l.count})</span></div>
                  </div>
                  <div style={{ height:4, background:"rgba(255,255,255,0.06)", borderRadius:3 }}>
                    <div style={{ height:"100%", width:pct+"%", background:"linear-gradient(90deg,#e8ff47,#a3e635)", borderRadius:3 }} />
                  </div>
                </div>
              );
            })}
          ...
          {/* Top markets */}
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"14px 16px" }}>
            <div style={{ fontSize:11, fontWeight:800, marginBottom:10, color:"rgba(255,255,255,0.6)", letterSpacing:1 }}>TOP BET MARKETS</div>
            {stats.topMarkets.map(function(m){
              var maxC = stats.topMarkets[0] ? stats.topMarkets[0].count : 1;
              var pct = Math.max(4, Math.round((m.count / maxC) * 100));
              return (
                <div key={m.market} style={{ marginBottom:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                    <div style={{ fontSize:11, fontWeight:700 }}>{m.market}</div>
                    <div style={{ fontSize:11, color:"#60a5fa", fontWeight:800 }}>{m.count} bets</div>
                  </div>
                  <div style={{ height:4, background:"rgba(255,255,255,0.06)", borderRadius:3 }}>
                    <div style={{ height:"100%", width:pct+"%", background:"linear-gradient(90deg,#60a5fa,#818cf8)", borderRadius:3 }} />
                  </div>
                </div>
              );
            })}
      ...
      {!loading && tab === "users" && (
        <div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)", marginBottom:10 }}>{users.length} registered users · sorted by wagered</div>
          {users.map(function(u){
            return (
              <div key={u.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"13px 15px", marginBottom:8 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:9 }}>
                    <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#e8ff47,#a3e635)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:900, color:"#060d1a", flexShrink:0 }}>{u.username[0].toUpperCase()}</div>
                    <div>
                      <div style={{ fontWeight:800, fontSize:13 }}>{u.username}</div>
                      <div style={{ fontSize:9, color:"rgba(255,255,255,0.3)", marginTop:1 }}>Joined {fmtDate(u.joined)}</div>
                    </div>
                  </div>
                  <button onClick={function(){ setBalanceModal(u); setBalanceAmt(""); }} style={{ padding:"6px 12px", background:"rgba(232,255,71,0.08)", border:"1px solid rgba(232,255,71,0.2)", color:"#e8ff47", borderRadius:8, fontWeight:800, fontSize:10, cursor:"pointer" }}>
                    Adjust Balance
                  </button>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6 }}>
                  {[
                    { l:"Balance", v: fmtAmt(u.balance), c:"#e8ff47" },
                    { l:"Wagered", v: fmtAmt(u.wagered), c:"#f87171" },
                    { l:"Won",     v: fmtAmt(u.won),     c:"#4ade80" },
                    { l:"Net P/L", v: fmtAmt(u.netPL),   c: u.netPL >= 0 ? "#4ade80" : "#f87171" },
                  ].map(function(s){
                    return <div key={s.l} style={{ background:"rgba(255,255,255,0.04)", borderRadius:8, padding:"7px 9px" }}>
                      <div style={{ fontSize:8, color:"rgba(255,255,255,0.3)", marginBottom:2 }}>{s.l}</div>
                      <div style={{ fontSize:12, fontWeight:800, color:s.c }}>{s.v}</div>
                    </div>;
                  })}
                </div>
                <div style={{ marginTop:7, display:"flex", gap:5 }}>
                  <span style={{ fontSize:9, padding:"2px 8px", background:"rgba(255,255,255,0.05)", borderRadius:5, color:"rgba(255,255,255,0.4)" }}>{u.totalBets} bets</span>
                  <span style={{ fontSize:9, padding:"2px 8px", background:"rgba(74,222,128,0.1)", borderRadius:5, color:"#4ade80" }}>{u.wins} wins</span>
                  {u.pending > 0 && <span style={{ fontSize:9, padding:"2px 8px", background:"rgba(251,191,36,0.1)", borderRadius:5, color:"#fbbf24" }}>{u.pending} pending</span>}
                </div>
              </div>
            );
          })}
      ...
      {!loading && tab === "bets" && (
        <div>
          <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap" }}>
            {["","pending","won","lost","refunded"].map(function(s){
              var labels = { "":"All", pending:"Pending", won:"Won", lost:"Lost", refunded:"Refunded" };
              var colors = { "":"#fff", pending:"#fbbf24", won:"#4ade80", lost:"#f87171", refunded:"#60a5fa" };
              return (
                <button key={s} onClick={function(){ setBetStatus(s); setBetsPage(1); }} style={{ padding:"5px 12px", borderRadius:7, border:"none", cursor:"pointer", fontWeight:800, fontSize:10, background: betStatus===s ? (colors[s]+"22") : "rgba(255,255,255,0.05)", color: betStatus===s ? colors[s] : "rgba(255,255,255,0.4)", outline: betStatus===s ? "1px solid " + colors[s] + "44" : "none" }}>
                  {labels[s]}
                </button>
              );
            })}
            <div style={{ marginLeft:"auto", fontSize:10, color:"rgba(255,255,255,0.3)", alignSelf:"center" }}>{betsTotal} total</div>
          </div>

          {bets.map(function(b){
            var sc = statusColor(b.status);
            return (
              <div key={b.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:11, padding:"11px 13px", marginBottom:7 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                  <div>
                    <div style={{ fontSize:10, color:"rgba(255,255,255,0.4)", marginBottom:2 }}>
                      <span style={{ color:"#e8ff47", fontWeight:700 }}>{b.username}</span> · {fmtDate(b.placed_at)}
                    </div>
                    <div style={{ fontWeight:800, fontSize:13 }}>{b.option_label}</div>
                    <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)", marginTop:1 }}>{b.match_label}</div>
                    <div style={{ fontSize:9, color:"rgba(255,255,255,0.2)", marginTop:1 }}>{b.market} · {b.league}</div>
                  </div>
                  <div style={{ textAlign:"right", flexShrink:0 }}>
                    <div style={{ fontSize:11, color:"#f87171", fontWeight:700 }}>{fmtAmt(b.amount)}</div>
                    <div style={{ fontSize:10, color:"#4ade80" }}>→ {fmtAmt(b.potential)}</div>
                    <div style={{ fontSize:9, color:"rgba(255,255,255,0.3)", marginTop:1 }}>{b.odds}x</div>
                  </div>
                </div>
                <span style={{ fontSize:9, padding:"2px 9px", background:sc+"18", color:sc, borderRadius:5, fontWeight:800, textTransform:"uppercase", letterSpacing:0.5 }}>{b.status}</span>
              </div>
            );
          })}
          ...
          <div style={{ display:"flex", gap:8, justifyContent:"center", marginTop:14 }}>
            <button disabled={betsPage<=1} onClick={function(){ setBetsPage(function(p){ return p-1; }); }} style={{ padding:"7px 16px", background:"rgba(255,255,255,0.06)", border:"none", color:"#fff", borderRadius:8, cursor:betsPage<=1?"not-allowed":"pointer", opacity:betsPage<=1?0.3:1, fontWeight:700 }}>← Prev</button>
            <div style={{ alignSelf:"center", fontSize:11, color:"rgba(255,255,255,0.4)" }}>Page {betsPage} of {Math.ceil(betsTotal/30)||1}</div>
            <button disabled={betsPage>=Math.ceil(betsTotal/30)} onClick={function(){ setBetsPage(function(p){ return p+1; }); }} style={{ padding:"7px 16px", background:"rgba(255,255,255,0.06)", border:"none", color:"#fff", borderRadius:8, cursor:betsPage>=Math.ceil(betsTotal/30)?"not-allowed":"pointer", opacity:betsPage>=Math.ceil(betsTotal/30)?0.3:1, fontWeight:700 }}>Next →</button>
          </div>
        </div>
      )}
      ...
      {balanceModal && (
        <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.85)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:"#0e1628", border:"1px solid rgba(232,255,71,0.2)", borderRadius:18, padding:24, width:"100%", maxWidth:360 }}>
            <div style={{ fontWeight:900, fontSize:16, marginBottom:4 }}>Adjust Balance</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.4)", marginBottom:16 }}>User: <strong style={{ color:"#e8ff47" }}>{balanceModal.username}</strong> · Current: <strong style={{ color:"#fff" }}>{fmtAmt(balanceModal.balance)}</strong></div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginBottom:6 }}>Amount (use negative to deduct)</div>
            <input
              type="number" value={balanceAmt} onChange={function(e){ setBalanceAmt(e.target.value); }}
              placeholder="e.g. 50 or -25"
              style={{ width:"100%", padding:"10px 13px", borderRadius:10, background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", color:"#fff", fontSize:14, outline:"none", marginBottom:10, boxSizing:"border-box" }}
            />
            <input
              type="text" value={balanceReason} onChange={function(e){ setBalanceReason(e.target.value); }}
              placeholder="Reason (optional)"
              style={{ width:"100%", padding:"10px 13px", borderRadius:10, background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", color:"#fff", fontSize:13, outline:"none", marginBottom:16, boxSizing:"border-box" }}
            />
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={function(){ setBalanceModal(null); }} style={{ flex:1, padding:12, background:"rgba(255,255,255,0.06)", border:"none", color:"rgba(255,255,255,0.6)", borderRadius:10, fontWeight:700, cursor:"pointer" }}>Cancel</button>
              <button onClick={handleBalanceAdjust} style={{ flex:1, padding:12, background:"#e8ff47", border:"none", color:"#060d1a", borderRadius:10, fontWeight:900, cursor:"pointer", fontSize:14 }}>Apply</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Action toast */}
      {actionMsg && <div style={{ position:"fixed", bottom:24, left:"50%", transform:"translateX(-50%)", background:"#0e1628", border:"1px solid " + actionMsg.color + "44", color:actionMsg.color, padding:"11px 20px", borderRadius:12, fontWeight:700, fontSize:13, zIndex:9999, whiteSpace:"nowrap", boxShadow:"0 8px 32px rgba(0,0,0,0.7)" }}>{actionMsg.msg}</div>}
    </div>
  );
}
...
export default function App() {
  ...
  var [balClickCount,setBalClickCount] = useState(0);
  var [balClickTimer,setBalClickTimer] = useState(null);
  var [showCodeModal,setShowCodeModal] = useState(false);
  var [codeInput,setCodeInput]         = useState("");
  var [codeShake,setCodeShake]         = useState(false);
  var [isAdmin,setIsAdmin]             = useState(function(){ return localStorage.getItem("bp_admin")==="1"; });
  ...
  var logout = function(){ localStorage.removeItem("bp_token"); localStorage.removeItem("bp_admin"); setUser(null); setMatches([]); setBetSlip([]); setIsAdmin(false); };

  var SECRET_CODE = "8055";
  var handleBalanceClick = function() {
    var newCount = balClickCount + 1;
    setBalClickCount(newCount);
    if (balClickTimer) clearTimeout(balClickTimer);
    var t = setTimeout(function(){ setBalClickCount(0); }, 2000);
    setBalClickTimer(t);
    if (newCount >= 10) {
      setBalClickCount(0);
      clearTimeout(t);
  ...
  var handleCodeSubmit = function() {
    if (codeInput === SECRET_CODE) {
      ...
      setCodeShake(true);
      setCodeInput("");
      setTimeout(function(){ setCodeShake(false); }, 600);
    }
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080d1a", fontFamily:"Trebuchet MS,sans-serif", color:"#fff", maxWidth:480, margin:"0 auto" }}>
      <style>{"@keyframes spin{to{transform:rotate(360deg)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.2}} @keyframes toastIn{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes shake{0%,100%{transform:translateX(0)}15%{transform:translateX(-8px)}30%{transform:translateX(8px)}45%{transform:translateX(-6px)}60%{transform:translateX(6px)}75%{transform:translateX(-3px)}90%{transform:translateX(3px)}} ::-webkit-scrollbar{width:2px} ::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1)} input::-webkit-outer-spin-button,input::-webkit-inner-spin-button{-webkit-appearance:none} input[type=number]{-moz-appearance:textfield}"}</style>
      ...
      <div style={{ display:"flex", gap:3, marginTop:9, background:"rgba(255,255,255,0.04)", borderRadius:11, padding:3 }}>
          ...
          {isAdmin && <TabBtn id="admin" label="⚙ Admin" />}
        </div>
      </div>
      ...
      <div style={{ padding:"12px 12px", paddingBottom:bottomPad }}>
        ...
        {page==="admin" && isAdmin && <AdminPanel token={localStorage.getItem("bp_token")} />}

      {showCodeModal && (
        <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.92)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:"#0b1526", border:"1px solid rgba(232,255,71,0.15)", borderRadius:20, padding:28, width:"100%", maxWidth:300, textAlign:"center" }}>
            <div style={{ fontSize:28, marginBottom:10 }}>🔐</div>
            <div style={{ fontWeight:900, fontSize:16, color:"#e8ff47", marginBottom:6 }}>Enter Code</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)", marginBottom:20 }}>Enter the admin access code</div>
            <input
              autoFocus
              type="password"
              value={codeInput}
              onChange={function(e){ setCodeInput(e.target.value); }}
              onKeyDown={function(e){ if(e.key==="Enter") handleCodeSubmit(); if(e.key==="Escape") { setShowCodeModal(false); setCodeInput(""); } }}
              maxLength={8}
              placeholder="••••"
              style={{ width:"100%", padding:"14px", borderRadius:12, background:"rgba(255,255,255,0.07)", border:codeShake?"1px solid #f87171":"1px solid rgba(255,255,255,0.12)", color:"#fff", fontSize:22, textAlign:"center", letterSpacing:8, outline:"none", boxSizing:"border-box", marginBottom:14, animation:codeShake?"shake 0.5s ease":"none" }}
            />
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={function(){ setShowCodeModal(false); setCodeInput(""); }} style={{ flex:1, padding:"11px", background:"rgba(255,255,255,0.05)", border:"none", color:"rgba(255,255,255,0.4)", borderRadius:10, fontWeight:700, cursor:"pointer" }}>Cancel</button>
              <button onClick={handleCodeSubmit} style={{ flex:1, padding:"11px", background:"#e8ff47", border:"none", color:"#060d1a", borderRadius:10, fontWeight:900, cursor:"pointer", fontSize:14 }}>Unlock</button>
            </div>
          </div>
        </div>
      )}

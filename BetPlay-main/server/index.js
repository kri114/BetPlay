...
// ─── ADMIN MIDDLEWARE ───────────────────────────────────────────────────────
const ADMIN_SECRET = process.env.ADMIN_SECRET || "8055";
function adminAuth(req, res, next) {
  const token = (req.headers.authorization || "").split(" ")[1];
  if (!token) return res.status(401).json({ error: "No token" });
  // Admin secret passed as extra header
...
app.get("/api/admin/stats", adminAuth, async (req, res) => {
  try {
    ...
    const totalPaidOut  = bets.filter(b => b.status === "won").reduce((s, b) => s + (b.potential || 0), 0);
    const totalRefunded = bets.filter(b => b.status === "refunded").reduce((s, b) => s + (b.amount || 0), 0);
    const houseEdge     = totalWagered - totalPaidOut - totalRefunded;

    const pendingCount  = bets.filter(b => b.status === "pending").length;
    const wonCount      = bets.filter(b => b.status === "won").length;
    const lostCount     = bets.filter(b => b.status === "lost").length;
    const refundedCount = bets.filter(b => b.status === "refunded").length;
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const dayEnd   = new Date(now - i * dayMs); dayEnd.setHours(23,59,59,999);
      const dayBets  = bets.filter(b => {
        const d = new Date(b.placed_at); return d >= dayStart && d <= dayEnd;
      });
      const wagered  = dayBets.reduce((s, b) => s + (b.amount || 0), 0);
      const paidOut  = dayBets.filter(b => b.status === "won").reduce((s, b) => s + (b.potential || 0), 0);
      dailyRevenue.push({
        ...
        profit:  Math.round((wagered - paidOut) * 100) / 100,
        bets:    dayBets.length,
      });
    }
    ...
    bets.forEach(b => {
      ...
      if (!leagueMap[l]) leagueMap[l] = { league: l, wagered: 0, count: 0 };
      leagueMap[l].wagered += b.amount || 0;
      leagueMap[l].count++;
    });
    const topLeagues = Object.values(leagueMap).sort((a,b) => b.wagered - a.wagered).slice(0, 8).map(l => ({ ...l, wagered: Math.round(l.wagered * 100) / 100 }));

    // Top markets
    const marketMap = {};
    bets.forEach(b => {
      const m = b.market || "Unknown";
      if (!marketMap[m]) marketMap[m] = { market: m, count: 0, wagered: 0 };
      marketMap[m].count++;
      marketMap[m].wagered += b.amount || 0;
    });
    const topMarkets = Object.values(marketMap).sort((a,b) => b.count - a.count).slice(0, 6).map(m => ({ ...m, wagered: Math.round(m.wagered * 100) / 100 }));

    res.json({
      overview: {
        totalUsers, totalBalance: Math.round(totalBalance*100)/100,
        totalWagered: Math.round(totalWagered*100)/100,
        totalPaidOut: Math.round(totalPaidOut*100)/100,
        totalRefunded: Math.round(totalRefunded*100)/100,
        houseEdge: Math.round(houseEdge*100)/100,
        totalBets: bets.length, pendingCount, wonCount, lostCount, refundedCount,
      },
      dailyRevenue, topLeagues, topMarkets,
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/admin/users", adminAuth, async (req, res) => {
  try {
    ...
    bets.forEach(b => {
      ...
      if (!betsByUser[uid]) betsByUser[uid] = [];
      betsByUser[uid].push(b);
    });
    const rows = users.map(u => {
      const ub = betsByUser[u._id.toString()] || [];
      const wagered = ub.reduce((s,b) => s + (b.amount||0), 0);
      const won     = ub.filter(b => b.status==="won").reduce((s,b) => s + (b.potential||0), 0);
      return {
        id: u._id.toString(), username: u.username, balance: u.balance,
        joined: u.joined, totalBets: ub.length,
        wagered: Math.round(wagered*100)/100, won: Math.round(won*100)/100,
        netPL: Math.round((won-wagered)*100)/100,
        wins: ub.filter(b=>b.status==="won").length,
        pending: ub.filter(b=>b.status==="pending").length,
      };
...
app.get("/api/admin/bets", adminAuth, async (req, res) => {
  try {
    ...
    const bets  = await db.collection("bets").find(query).sort({ placed_at: -1 }).skip((page-1)*limit).limit(limit).toArray();
    // Attach usernames
    ...
    const rows = bets.map(b => ({ ...b, id: b._id.toString(), userId: b.userId.toString(), username: userMap[b.userId.toString()] || "?" }));
    res.json({ bets: rows, total, page, pages: Math.ceil(total / limit) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post("/api/admin/users/:id/balance", adminAuth, async (req, res) => {
  try {
    ...
    const newBalance = Math.round((user.balance + amount) * 100) / 100;
    await db.collection("users").updateOne({ _id: user._id }, { $set: { balance: newBalance } });
    res.json({ ok: true, newBalance, username: user.username });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Force settle all pending bets

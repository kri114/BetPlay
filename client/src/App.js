...
function PlayerRow({ p }) {
  return <div style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
    <div style={{ width:22, height:22, borderRadius:"50%", background:"rgba(255,255,255,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:"rgba(255,255,255,0.4)", flexShrink:0 }}>{p.shirtNumber || "-"}</div>
...
function MatchModal({ match: m, onClose, onBet, balance, favMatches, onToggleFavMatch, onMatchUpdate }) {
  ...
  var [currentMatch, setCurrentMatch] = useState(m);

  // Load match detail (stats + lineups) once when opening stats or lineups tabs
  var loadDetail = useCallback(function() {
    if (detail || detailLoad) return;
  ...
  }, [detail, detailLoad, m.fixtureId]);

  // Auto-refresh detail every 30s when live
  useEffect(function() {
    if (tab !== "stats" && tab !== "lineups") return;
    loadDetail();
    if (!isLive) return;
  ...
  var doPlace = function() {
    ...
    onBet(currentMatch, slip.optLabel, a, slip.odds, slip.marketLabel);
    onClose();
  };
        ...
        <div style={{ padding:"16px 16px 0", background:"linear-gradient(160deg,"+lc+"20,transparent 60%)", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:2 }}>
            {["markets","lineups","stats"].map(function(t) {
              return <button key={t} onClick={function(){ setTab(t); if(t!=="markets") loadDetail(); }} style={{ padding:"8px 16px", background:"none", border:"none", borderBottom:tab===t?"2px solid "+lc:"2px solid transparent", color:tab===t?"#fff":"rgba(255,255,255,0.35)", cursor:"pointer", fontWeight:700, fontSize:12, textTransform:"capitalize" }}>{t}</button>;
            })}
          </div>
        </div>
          ...
          {tab === "markets" && (
            <div style={{ padding:"0 14px 16px" }}>
              {markets.map(function(market) {
                var cols = market.options.length === 2 ? "1fr 1fr"
                  : market.options.length === 3 ? "1fr 1fr 1fr"
                  : "1fr 1fr 1fr 1fr";
                return (
              ...
              {!detailLoad && !detailErr && !hasLineups && (
                <div style={{ textAlign:"center", padding:"30px 20px" }}>
                  ...
                  <div style={{ fontSize:12, color:"rgba(255,255,255,0.28)", lineHeight:1.7 }}>
                    {!isLive && !isFT
                      ? "Lineups are usually published 1 hour before kick-off"
            ...
            <div style={{ padding:"14px" }}>
              ...
              {(isLive || isFT) && detail && (
                <div>
                  ...
                  {detail.fullTime && (
                    <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:12, padding:"12px 16px", marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        ...
                        <span style={{ fontSize:28, fontWeight:900, letterSpacing:4 }}>{detail.fullTime.home} - {detail.fullTime.away}</span>
                  ...
                  {/* Referee */}
                  {detail.referees && <div style={{ fontSize:11, color:"rgba(255,255,255,0.25)", marginBottom:8 }}>Referee: {detail.referees}</div>}

                  {goals.length === 0 && <div style={{ textAlign:"center", padding:16, color:"rgba(255,255,255,0.25)", fontSize:12 }}>No goals scored yet</div>}
                </div>
              )}
            </div>
          )}
...
export default function App() {
  ...
  useEffect(function() {
    ...
    api("/me")
      ...
      .catch(function() { localStorage.removeItem("bp_token"); })
      .finally(function() { setAuthChecked(false); });
  }, []);

  // Countdown for ad cooldown
  useEffect(function() {
  ...
  // Refresh user balance/bets to pick up settlements
  ...
  useEffect(function() {
    ...
    // Refresh matches every 30s (was 60s) to keep live scores current
    refreshRef.current = setInterval(function() {
      fetchMatches();
      refreshUser(); // also refresh balance in case bets settled
    }, 30000);
  ...
  var handleBet = function(match, optionLabel, amount, odds, market) {
    ...
    .then(function(d) {
      ...
      // Refresh balance after 5s in case it was already finished
  ...
  if (!authChecked && !user) return null; // brief flash prevention
  ...
  var rightMatches = activeLeague==="live" ? liveMatches
    : activeLeague==="favorites" ? matches.filter(function(m){ return favMatches.has(m.id); })
    : matches.filter(function(m){ return m.leagueId===activeLeague; });
  ...
  var MatchCard = function(props) {
    var m = props.m, lc = m.leagueColor;
      ...
      <div style={{ background:"rgba(255,255,255,0.04)", borderLeft:"3px solid "+lc, border:"1px solid rgba(255,255,255,0.07)", borderRadius:13, padding:"13px 14px", marginBottom:8 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:7 }}>
            ...
            <button onClick={function(){ toggleFavMatch(m.id); }} style={{ background:"none", border:"none", fontSize:13, cursor:"pointer", padding:0, color:favMatches.has(m.id)?"#e8ff47":"rgba(255,255,255,0.2)" }}>{favMatches.has(m.id)?"*":"o"}</button>
          </div>
        </div>
      ...
      <div style={{ padding:"12px 12px 80px" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <div style={{ width:68, flexShrink:0, overflowY:"auto", maxHeight:"calc(100vh - 180px)" }}>
              ...
              {allLeagues.map(function(l) {
                var isActive = activeLeague===l.id, lc = l.color;
                return (
                  <div key={l.id} style={{ marginBottom:5, position:"relative" }}>
                    ...
                    <button onClick={function(e){ e.stopPropagation(); toggleFavLeague(l.id); }} style={{ position:"absolute", top:2, right:2, background:"none", border:"none", fontSize:8, cursor:"pointer", padding:0, color:favLeagues.has(l.id)?"#e8ff47":"rgba(255,255,255,0.18)" }}>{favLeagues.has(l.id)?"*":"o"}</button>
                  </div>
                );
              })}
            </div>
        ...
        {page==="profile" && (
          <div>
            ...
            <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:15, marginBottom:14 }}>
              <div style={{ fontWeight:800, fontSize:13, marginBottom:4 }}>About Account Persistence</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.35)", lineHeight:1.7 }}>
                Accounts are stored on the server. On Render's free tier the server may restart occasionally which resets accounts. To keep accounts permanently, the Render service needs a Persistent Disk added at /data with DB_PATH=/data/betplay_db.json.
              </div>
            </div>

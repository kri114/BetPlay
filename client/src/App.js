...
function getAllMarkets(m) {
  ...
  return [
    ...
    { id: "ht", label: "Half-Time Result", options: [
      { label: m.home + " HT", odds: +(h * 1.35).toFixed(2) },
      { label: "HT Draw", odds: 2.05 },
      { label: m.away + " HT", odds: +(a * 1.35).toFixed(2) },
    ]},
    ...
    { id: "anytime", label: "Anytime Scorer", options: [
      { label: m.home + " scores", odds: Math.max(1.10, +(h * 0.65).toFixed(2)) },
      { label: m.away + " scores", odds: Math.max(1.10, +(a * 0.65).toFixed(2)) },
    ]},
...
function Loader() {
  return (
    <div style={{ textAlign: "center", padding: "30px 0", color: "rgba(255,255,255,0.3)" }}>
      <div style={{ width: 24, height: 24, border: "3px solid rgba(255,255,255,0.1)", borderTop: "3px solid #e8ff47", borderRadius: "50%", margin: "0 auto 8px", animation: "spin 0.8s linear infinite" }} />
    </div>
  );
}
...
function MatchModal({ match: m, onClose, onBet, balance, favMatches, onToggleFavMatch }) {
  ...
  var [slip, setSlip] = useState(null); // { marketId, marketLabel, optLabel, odds }
  var [amt, setAmt] = useState("");
  var [tab, setTab] = useState("markets"); // markets | stats
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ width: "100%", maxWidth: 480, background: "#0b1120", borderRadius: "22px 22px 0 0", maxHeight: "96vh", display: "flex", flexDirection: "column" }} onClick={function(e) { e.stopPropagation(); }}>
        ...
        <div style={{ padding: "16px 16px 0", background: "linear-gradient(160deg," + lc + "20,transparent 60%)", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button onClick={function() { onToggleFavMatch(m.id); }} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", padding: 0, lineHeight: 1 }}>{isFav ? "[star]" : "[o]"}</button>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 14 }}>
            <div style={{ flex: 1, textAlign: "center" }}>
              {m.homeLogo && <img src={m.homeLogo} alt="" style={{ width: 40, height: 40, objectFit: "contain", marginBottom: 5 }} onError={function(e) { e.target.style.display = "none"; }} />}
            ...
            <div style={{ flex: 1, textAlign: "center" }}>
              {m.awayLogo && <img src={m.awayLogo} alt="" style={{ width: 40, height: 40, objectFit: "contain", marginBottom: 5 }} onError={function(e) { e.target.style.display = "none"; }} />}
          ...
          <div style={{ display: "flex", gap: 2 }}>
            {["markets", "stats"].map(function(t) {
              return <button key={t} onClick={function() { setTab(t); }} style={{ padding: "8px 18px", background: "none", border: "none", borderBottom: tab === t ? "2px solid " + lc : "2px solid transparent", color: tab === t ? "#fff" : "rgba(255,255,255,0.35)", cursor: "pointer", fontWeight: 700, fontSize: 12, textTransform: "capitalize" }}>{t}</button>;
            })}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 0 8px 0" }}>
              ...
              {markets.map(function(market) {
                ...
                return (
                  <div key={market.id} style={{ marginTop: 14 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 7, textTransform: "uppercase" }}>{market.label}</div>
                    <div style={{ display: "grid", gridTemplateColumns: cols, gap: 6 }}>
                      {market.options.map(function(opt, i) {
                        var isSelected = slip && slip.marketId === market.id && slip.optLabel === opt.label;
                        return (
                          <button key={i} onClick={function() {
                            if (isSelected) { setSlip(null); }
                            else { setSlip({ marketId: market.id, marketLabel: market.label, optLabel: opt.label, odds: opt.odds }); }
                          }} style={{
                            padding: "9px 4px", borderRadius: 10, cursor: "pointer", textAlign: "center",
                            border: isSelected ? "2px solid " + lc : "1px solid rgba(255,255,255,0.09)",
                            background: isSelected ? lc + "22" : "rgba(255,255,255,0.04)",
                            transition: "all 0.15s",
                          }}>
                            <div style={{ fontSize: 9, color: isSelected ? lc : "rgba(255,255,255,0.4)", marginBottom: 4, lineHeight: 1.3 }}>{opt.label}</div>
                            <div style={{ fontSize: 16, fontWeight: 900, color: isSelected ? lc : "#fff" }}>{opt.odds}x</div>
                          </button>
                        );
                      })}
                    </div>
          ...
          {tab === "stats" && (
            <div style={{ padding: "14px 14px" }}>
              {statsLoading ? <Loader /> : !(isLive || isFT) ? (
                <div style={{ textAlign: "center", padding: "30px 16px" }}>
                  <div style={{ fontSize: 32, marginBottom: 10 }}>&#9201;</div>
                  <div style={{ fontWeight: 700, color: "rgba(255,255,255,0.5)", marginBottom: 6 }}>Match not started yet</div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.28)" }}>Stats available after kick-off</div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.28)", marginTop: 4 }}>{m.time}</div>
                </div>
              ) : (
                <div>
                  ...
                  {stats && stats.stats && stats.stats.length > 0 ? (
                    <div>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, fontWeight: 800, color: "rgba(255,255,255,0.3)", marginBottom: 10 }}>
                        <span>{m.home}</span><span>STATS</span><span>{m.away}</span>
                      </div>
                      <StatBar label="Ball Possession" a={parseInt(sv(0, "Ball Possession")) || 50} b={parseInt(sv(1, "Ball Possession")) || 50} />
                      <StatBar label="Total Shots" a={sv(0, "Total Shots")} b={sv(1, "Total Shots")} />
                      <StatBar label="Shots on Target" a={sv(0, "Shots on Target")} b={sv(1, "Shots on Target")} />
                      <StatBar label="Corner Kicks" a={sv(0, "Corner Kicks")} b={sv(1, "Corner Kicks")} />
                      ...
                      <StatBar label="Pass Accuracy" a={parseInt(sv(0, "Passes %")) || 0} b={parseInt(sv(1, "Passes %")) || 0} />
                    </div>
                  ) : (
                    <div style={{ textAlign: "center", padding: 20, color: "rgba(255,255,255,0.3)", fontSize: 12 }}>Stats not available for this match</div>
                  )}
                </div>
              )}
            </div>
        ...
        {/* Sticky Bet Slip at bottom */}
        {slip && (
          <div style={{ flexShrink: 0, borderTop: "1px solid " + lc + "44", background: "#0d1628", padding: "12px 14px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ flex: 1, position: "relative" }}>
                <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.4)", fontSize: 14, fontWeight: 700 }}>$</span>
      ...
      <div style={{ width: "100%", maxWidth: 360, background: "rgba(255,255,255,0.04)", borderRadius: 22, padding: 28, border: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", gap: 4, marginBottom: 22, background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: 4 }}>
          {["login", "signup"].map(function(m) {
            return <button key={m} onClick={function() { setMode(m); setError(""); }} style={{ flex: 1, padding: "9px", background: mode === m ? "#e8ff47" : "none", color: mode === m ? "#060d1a" : "rgba(255,255,255,0.45)", border: "none", borderRadius: 9, cursor: "pointer", fontWeight: 800, fontSize: 13 }}>{m === "login" ? "Log In" : "Sign Up"}</button>;
          })}
        </div>
...
export default function App() {
  ...
  var handleBet = function(match, optionLabel, amount, odds, market) {
    api("/bet", { method: "POST", body: {
      ...
      optionLabel: optionLabel, market: market,
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", borderLeft: "3px solid " + lc, border: "1px solid rgba(255,255,255,0.07)", borderRadius: 13, padding: "13px 14px", marginBottom: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: isLive ? "#ef4444" : isFT ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.3)" }}>
              {isLive ? ("LIVE " + (m.elapsed ? m.elapsed + "'" : "")) : isFT ? "FT" : m.time}
            </span>
            <button onClick={function() { toggleFavMatch(m.id); }} style={{ background: "none", border: "none", fontSize: 13, cursor: "pointer", padding: 0, color: favMatches.has(m.id) ? "#e8ff47" : "rgba(255,255,255,0.2)" }}>{favMatches.has(m.id) ? "[star]" : "[o]"}</button>
          </div>
        </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ width: 68, flexShrink: 0, overflowY: "auto", maxHeight: "calc(100vh - 180px)" }}>
              ...
              <div onClick={function() { setActiveLeague("favorites"); }} style={{ marginBottom: 5, padding: "7px 3px", borderRadius: 10, cursor: "pointer", textAlign: "center", background: activeLeague === "favorites" ? "rgba(232,255,71,0.1)" : "rgba(255,255,255,0.04)", border: activeLeague === "favorites" ? "1px solid rgba(232,255,71,0.25)" : "1px solid rgba(255,255,255,0.06)" }}>
                ...
                {favMatches.size > 0 && <div style={{ fontSize: 11, color: "#e8ff47" }}>{favMatches.size}</div>}
              </div>
              {allLeagues.map(function(l) {
                ...
                return (
                  <div key={l.id} style={{ marginBottom: 5, position: "relative" }}>
                    ...
                    <button onClick={function(e) { e.stopPropagation(); toggleFavLeague(l.id); }} style={{ position: "absolute", top: 2, right: 2, background: "none", border: "none", fontSize: 8, cursor: "pointer", padding: 0, color: favLeagues.has(l.id) ? "#e8ff47" : "rgba(255,255,255,0.18)" }}>{favLeagues.has(l.id) ? "[star]" : "[o]"}</button>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {fetchError && (
                <div style={{ padding: "10px 12px", background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 10, marginBottom: 10, fontSize: 11, color: "#f87171", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {!loading && rightMatches.length === 0 && !fetchError && (
                <div style={{ textAlign: "center", padding: "40px 16px" }}>
                  <div style={{ fontWeight: 700, color: "rgba(255,255,255,0.4)", marginBottom: 6 }}>
                    {activeLeague === "live" ? "No live matches right now" : activeLeague === "favorites" ? "No favourited matches" : "No matches"}
                  </div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.2)" }}>
                    {activeLeague === "live" ? "Check back during match times" : activeLeague === "favorites" ? "Tap the star on any match card" : "No upcoming or recent matches"}
                  </div>
                </div>
              )}
